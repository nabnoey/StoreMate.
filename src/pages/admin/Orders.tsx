...
function Orders() {
  ...
  const handleSelectOrder = (orderNo: number) => {
    setSelectedOrders((prev) =>
      prev.includes(orderNo) ? prev.filter((id) => id !== orderNo) : [...prev, orderNo]
    );
  };

  const handleConfirm = async (order: OrderMod) => {
  try {
    // อัปเดตสถานะก่อน
    console.log("ยืนยันออเดอร์", order);
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
          ...
          <div className="flex flex-col gap-4 mb-6">
            <div className="flex items-center gap-3">
              <button type="button" className="h-[40px] px-4 bg-blue-700 hover:bg-blue-800 text-white rounded-lg text-sm font-medium transition-colors">
                ปริ้นใบปะหน้าที่เลือก ({selectedOrders.length})
              </button>
              <button
               onClick={() => handleConfirm(orders[0])} 
               type="button" className="h-[40px] w-[100px] bg-blue-700 hover:bg-blue-800 text-white rounded-lg text-sm font-medium transition-colors">
                ยืนยัน
              </button>
              <button type="button" className="h-[40px] w-[100px] rounded-lg border border-black text-gray-700 text-sm font-semibold font-['Anuphan']">
                ยกเลิก
              </button>
            </div>
                    ...
                    return (
                      <tr key={order.id || order.orderNo} className="hover:bg-gray-50/50 transition-colors">
                          ...
                          <div className="flex items-center gap-3">
                            <button
                              ...
                              className={`w-5 h-5 rounded-full border flex items-center justify-center transition-colors ${
                                isSelected ? "bg-blue-700 border-blue-700" : "border-gray-300 bg-white"
                              }`}
                            >
                        ...
                        <td className="py-4 text-right text-xs space-x-3 pr-2">
                          {order.is_printed && <span className="text-[#60A5FA] text-xs font-medium">printed</span>}
                          <button
                            type="button"
                ...
                visiblePages.map((page) => (
                  <button
                    ...
                    className={`w-7 h-7 rounded flex items-center justify-center font-medium transition-colors ${
                      page === currentPage ? "text-blue-600 font-bold bg-transparent" : "text-gray-500 hover:bg-gray-100"
                    }`}
                  >
                    {page}
                  </button>
