...
const Navbar: React.FC = () => {
  ...
  const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value.trim() !== "") {
      dispatch(
        search({ keyword: value, category: "", minPrice: 0, maxPrice: 0 })
      );
    }
  };
    ...
    <nav className="flex items-center justify-between bg-white shadow-sm h-[60px] lg:h-[101px] px-4 lg:px-10 relative">
      ...
      <div className="navbar-start flex items-center">
        <Link to="/">
          <img
            src={logo}
            className="w-27 lg:w-38 cursor-pointer mt-5 -ml-8"
            alt="Logo"
            data-test="logo"
          />
        </Link>
        ...
        {isAuthenticated ? (
          <>
            <div className="flex gap-3 lg:gap-4 items-center text-gray-600">
              ...
              >
                <FaCartShopping size={22} className="hover:text-black" />
      ...
      {/* MOBILE MENU */}
      {openMenu && (
        <div className="absolute top-[60px] right-4 w-[300px] bg-white z-30 lg:hidden rounded-none overflow-hidden">
          ...
          <div className="flex flex-col py-2">
            <Link
              ...
              className="px-6 py-4 text-gray-700 font-medium hover:bg-blue-50"
            >
              สินค้า
            </Link>
